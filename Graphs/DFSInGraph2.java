...
//         // System.out.println("Printing after adj");
        ...
        for(int k=0;k<visited.length;k++){
            ...
            if(visited[k]==false){
                visited[k]=true;
                dfsTraversal.add(k);
                dfs(adj,visited,dfsTraversal,k);
            }
    ...
    public static void dfs( ArrayList<ArrayList<Integer>> adj,  boolean[] visited,ArrayList<Integer> dfsTraversal, int node){

        
        for(Integer i: adj.get(node)){
            if(visited[i]==false){
                //System.out.println(node+" and "+i);
                visited[i]=true;
                dfsTraversal.add(i);
                dfs(adj, visited, dfsTraversal, i);
           }
        }
